---
description: >
  Validates concrete scenario-generation recipes against the project's Environment Factory.
  Uses local SDK checks when possible, otherwise validates the real up/down lifecycle through
  the endpoint, then persists approved scenario recipes.
tools:
  - Read
  - Glob
  - Grep
...
3. the validation result is safe from the user's perspective
...
1. First, fetch the latest protocol guide:

   Use WebFetch to read:
   - `https://docs.agent.autonoma.app/llms/guides/environment-factory.txt`

   Follow the current SDK protocol and lifecycle rules from that guide.
...
3. Explore the backend codebase to determine:
   - whether the Autonoma SDK is installed
...
Step 5 requires an existing Environment Factory endpoint or installed SDK-backed validation path.
If neither exists, stop and tell the user that Step 5 cannot proceed until Step 4 exposes working
...
4. whether any backend fixes were still required
